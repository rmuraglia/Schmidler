...
# collect data for seqBAR vs AIS comparison figure
...
print_out<-matrix(data=c(rep, smc_meth, targ_dist, num_distr, path_height, numtraj, prc_F, nummetrostep, estimate[1]))

# write to file
write.table(print_out, file='sbar_v_ais_out.txt', append=TRUE, quote=F, sep='\t', col.names=F, row.names=F)
