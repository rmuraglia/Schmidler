# varBAR_dijkstra_unibi_master.r
...
U1<-function() {
    potential<-(x^4-81*x^2)/150
    return(potential)
}

Umaker<-function(lambda) {
    newU<-(1-lambda)*U0(mu0) + lambda*U1()
    # newU<-(1-lambda)*U0(mu0) + lambda*U0(5)
    return(newU)
}
...
ggsave(file='varBAR-unibi-paramview.pdf', plot=paramplot1)
