        ...
        for (j in 1:length(query_nodes)) {
            ...
            for (l in 1:length(j_neighbors)) {
                ...
                if (is.na(D[j_ind, neigh_ind])) {
                    ...
                    neigh_rest<-dist_move(coords1, coords2)
...
# reconstruct solutions path from the output of k_node_search()
...
path_soln<-function(trace_mat, num_node) {
    ...
    colnames(seq_out)<-c('lambda','temperature')
