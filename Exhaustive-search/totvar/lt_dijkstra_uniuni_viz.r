# lt_dijkstra_uniuni_viz.r

# visualize results for dijkstra search
...
path.cost<-function(path) { #get total cost of a path
    ...
    for (i in 2:length(path)) {
        A.ind<-which(indexer==paste(path[[i-1]], collapse='_'))
        A<-ann.map[[A.ind]]
        B<-path[[i]][1]
...
bad.indexer<-c('0_1','0.2_1','0.4_1','0.6_1','0.8_1','1_1')
...
bad.path<-list(c(0,1)); bad.path[[2]]<-c(0.2,1); bad.path[[3]]<-c(0.4,1)
bad.path[[4]]<-c(0.6,1); bad.path[[5]]<-c(0.8,1); bad.path[[6]]<-c(1,1)
...
bpath<-dpath
bpath[,1]<-seq(from=0, to=1, by=0.2)
bpath[,2]<-1
bpath[,3]<-'Standard'
...
png('unimod-3pan-searchv2.png', width=15, height=5, units='in',res=600)
